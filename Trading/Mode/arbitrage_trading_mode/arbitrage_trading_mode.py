...
from octobot.constants import OCTOBOT_CHANNEL
...
from octobot_trading.api.exchange import get_all_exchange_ids_with_same_matrix_id
from octobot_trading.channels.exchange_channel import get_chan
from octobot_trading.channels.orders import OrdersChannel
from octobot_trading.constants import MODE_CHANNEL, MARK_PRICE_CHANNEL, ORDER_DATA_FETCHING_TIMEOUT
from octobot_trading.consumers.abstract_mode_consumer import AbstractTradingModeConsumer
from octobot_trading.consumers.octobot_channel_consumer import OctoBotChannelTradingActions, \
    OctoBotChannelTradingDataKeys
from octobot_trading.enums import ExchangeConstantsOrderColumns, OrderStatus, EvaluatorStates, TraderOrderType, \
...
from octobot_trading.orders.order_util import get_pre_order_data, total_fees_from_order_dict
...
class ArbitrageTradingMode(AbstractTradingMode):
    ...
    async def create_producers(self) -> list:
        mode_producer = ArbitrageModeProducer(get_chan(MODE_CHANNEL, self.exchange_manager.id),
    ...
    async def _order_notification_callback(self, exchange, exchange_id, cryptocurrency, symbol, order,
        ...
        elif order[ExchangeConstantsOrderColumns.STATUS.value] == OrderStatus.CANCELED.value and is_from_bot:
    ...
    def get_is_trading_on_exchange(cls, exchange_name) -> bool:
        ...
        """
        exchanges_to_trade_on = get_tentacle_config(cls).get("exchanges_to_trade_on", [])
...
class ArbitrageModeConsumer(AbstractTradingModeConsumer):
    ...
    async def _create_initial_arbitrage_order(self, arbitrage_container):
        current_symbol_holding, current_market_holding, market_quantity, price, symbol_market = \
            await get_pre_order_data(self.exchange_manager,
                                     symbol=self.trading_mode.symbol,
                                     timeout=ORDER_DATA_FETCHING_TIMEOUT)

        created_orders = []
        order_type = TraderOrderType.BUY_LIMIT if arbitrage_container.state is EvaluatorStates.LONG \
            else TraderOrderType.SELL_LIMIT
        quantity = self._get_quantity_from_holdings(current_symbol_holding, market_quantity, arbitrage_container.state)
        if order_type is TraderOrderType.SELL_LIMIT:
    ...
    async def _create_secondary_arbitrage_order(self, arbitrage_container, quantity):
        ...
        current_symbol_holding, current_market_holding, market_quantity, price, symbol_market = \
            await get_pre_order_data(self.exchange_manager,
                                     symbol=self.trading_mode.symbol,
                                     timeout=ORDER_DATA_FETCHING_TIMEOUT)
        now_selling = arbitrage_container.state is EvaluatorStates.LONG
        if now_selling:
            quantity = add_dusts_to_quantity_if_necessary(quantity, price, symbol_market, current_symbol_holding)
        for order_quantity, order_price in check_and_adapt_order_details_if_necessary(
                quantity,
                ...
                symbol_market):
            current_order = create_order_instance(trader=self.exchange_manager.trader,
                                                  order_type=TraderOrderType.SELL_LIMIT if now_selling
            ...
            current_order = create_order_instance(trader=self.exchange_manager.trader,
                                                  ...
                                                  side=TradeOrderSide.SELL if now_selling else TradeOrderSide.BUY)
    ...
    def _get_quantity_from_holdings(self, current_symbol_holding, market_quantity, state):
        ...
        if state is EvaluatorStates.LONG:
    ...
    def _get_stop_loss_price(self, symbol_market, starting_price, now_selling):
        if now_selling:
            return adapt_price(symbol_market, starting_price * (1 - self.STOP_LOSS_DELTA_FROM_OWN_PRICE))
        return adapt_price(symbol_market, starting_price * (1 + self.STOP_LOSS_DELTA_FROM_OWN_PRICE))


class ArbitrageModeProducer(AbstractTradingModeProducer):
    ...
    def __init__(self, channel, config, trading_mode, exchange_manager):
        ...
        self.final_eval = ""
        self.quote, self.base = split_symbol(self.trading_mode.symbol)
        self.lock = Lock()
    ...
    async def start(self) -> None:
        ...
        """
        for exchange_id in get_all_exchange_ids_with_same_matrix_id(self.exchange_manager.exchange_name,
        ...
        await get_chan(MARK_PRICE_CHANNEL, self.exchange_manager.id).new_consumer(
            self._own_exchange_mark_price_callback,
            symbol=self.trading_mode.symbol
        )
        await get_chan(OCTOBOT_CHANNEL, self.trading_mode.bot_id).new_consumer(
            # listen for new available exchange
            self._exchange_added_callback,
            subject=OctoBotChannelSubjects.NOTIFICATION.value,
            action=OctoBotChannelTradingActions.EXCHANGE.value
        )
    ...
    async def order_filled_callback(self, filled_order):
        ...
        :return: None
        """
        order_id = filled_order[ExchangeConstantsOrderColumns.ID.value]
        async with self.lock:
            arbitrage = self._get_arbitrage(order_id)
            if arbitrage is not None:
                filled_quantity = filled_order[ExchangeConstantsOrderColumns.FILLED.value]
                if arbitrage.passed_initial_order:
                    # filled limit order or stop loss: close arbitrage
                    arbitrage_success = filled_order[ExchangeConstantsOrderColumns.TYPE.value] != \
                                        TradeOrderType.STOP_LOSS.value
                    if arbitrage.state is EvaluatorStates.LONG:
                        filled_quantity = filled_quantity * filled_order[ExchangeConstantsOrderColumns.PRICE.value]
    ...
    async def order_cancelled_callback(self, cancelled_order):
        ...
        :return: None
        """
        order_id = cancelled_order[ExchangeConstantsOrderColumns.ID.value]
        async with self.lock:
    ...
    async def _analyse_arbitrage_opportunities(self):
        other_exchanges_average_price = mean(self.other_exchanges_mark_prices.values())
        state = None
    ...
    async def _trigger_arbitrage_opportunity(self, other_exchanges_average_price, state):
        ...
        if self._ensure_no_existing_arbitrage_on_this_price(state):
            ...
            arbitrage_container = ArbitrageContainer(self.own_exchange_mark_price, other_exchanges_average_price, state)
    ...
    async def _trigger_arbitrage_secondary_order(self, arbitrage, filled_order, filled_quantity_before_fees):
        ...
        now_buying = arbitrage.state is EvaluatorStates.SHORT
        ...
        paid_fees_in_quote = total_fees_from_order_dict(filled_order, self.quote)
        ...
        if now_buying:
            ...
            fees_in_base = total_fees_from_order_dict(filled_order, self.base)
    ...
    def _log_results(self, arbitrage, success, filled_quantity):
        self.logger.info(f"Closed {arbitrage.state.name} arbitrage on {self.exchange_manager.exchange_name} ["
                         ...
                         f"profit before {'final' if arbitrage.state is EvaluatorStates.SHORT else 'all'} fees: "
                         f"{filled_quantity - arbitrage.initial_before_fee_filled_quantity} "
                         f"{self.quote if arbitrage.state is EvaluatorStates.SHORT else self.base}")
    ...
    async def _exchange_added_callback(self, bot_id: str, subject: str, action: str, data: dict):
        if OctoBotChannelTradingDataKeys.EXCHANGE_ID.value in data:
            # New exchange available: subscribe to its price updates
            await self._subscribe_exchange_id_mark_price(data[OctoBotChannelTradingDataKeys.EXCHANGE_ID.value])

    async def _subscribe_exchange_id_mark_price(self, exchange_id):
        await get_chan(MARK_PRICE_CHANNEL, exchange_id).new_consumer(
