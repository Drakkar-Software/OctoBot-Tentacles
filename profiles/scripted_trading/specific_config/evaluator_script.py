...
async def script(ctx: Context):
    ...
    required_candles_count = await user_input(ctx, "required_candles_count", "int", 15)
    ...
    value = await ctx.get_cached_value(ema_rising_title, tentacle_name="ScriptedEvaluator")
    values = await ctx.get_cached_values(ema_rising_title, limit=50, tentacle_name="ScriptedEvaluator")
