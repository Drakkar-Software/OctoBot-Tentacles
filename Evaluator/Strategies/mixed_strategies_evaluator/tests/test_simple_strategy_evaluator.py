...
class SimpleStrategyEvaluatorTest(abstract_strategy_test.AbstractStrategyTest):
    ...
    async def test_default_run(self):
        ...
        await self.run_test_default_run(decimal.Decimal(str(-9.26)))
    ...
    async def test_slow_downtrend(self):
        ...
        await self.run_test_slow_downtrend(decimal.Decimal(str(-9.26)), decimal.Decimal(str(-29.321)),
                                           decimal.Decimal(str(-16.191)), decimal.Decimal(str(-15.864)))

    async def test_sharp_downtrend(self):
        # market: -30.271723049610415
        # market: -32.091097308488614
        await self.run_test_sharp_downtrend(decimal.Decimal(str(-7.578)), decimal.Decimal(str(-22.09)))
    ...
    async def test_flat_markets(self):
        ...
        await self.run_test_flat_markets(decimal.Decimal(str(2.429)), decimal.Decimal(str(8.981)),
                                         decimal.Decimal(str(-5.819)), decimal.Decimal(str(-18.693)))

    async def test_slow_uptrend(self):
        # market: 32.524679029957184
        # market: 6.25
        await self.run_test_slow_uptrend(decimal.Decimal(str(4.95)), decimal.Decimal(str(5.348)))

    async def test_sharp_uptrend(self):
        # market: 24.56254050550875
        # market: 8.665472458575891
        await self.run_test_sharp_uptrend(decimal.Decimal(str(8.54)), decimal.Decimal(str(9.182)))

    async def test_up_then_down(self):
        # market: 1.1543668450702853
        await self.run_test_up_then_down(decimal.Decimal(str(8.714)))
