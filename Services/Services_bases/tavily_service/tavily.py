...
class TavilyService(services.AbstractService):
    """OctoBot service for Tavily API (search, extract, crawl, map, research)."""
    ...
    async def _post(self, path: str, data: dict, timeout: float = 60) -> dict:
        """POST request to Tavily API; returns JSON as dict. On error returns {} and logs."""
    ...
    async def _get(self, path: str, timeout: float = 30) -> dict:
        """GET request to Tavily API; returns JSON as dict. On error returns {} and logs."""
        if not self._api_key:
            if self.logger:
                self.logger.error("Tavily API key not set")
            return {}
    ...
    async def search(
        ...
        """POST /search — Web search. query is required."""
        data = {"query": query}
        ...
        return await self._post("search", data, timeout=min(timeout, 120))
    ...
    async def extract(
        ...
        """POST /extract — Content extraction from URLs. urls can be list or single string."""
        ...
        if chunks_per_source is not None:
            data["chunks_per_source"] = chunks_per_source
        return await self._post("extract", data, timeout=timeout)
    ...
    async def crawl(
        ...
        """POST /crawl — Crawl and extract from URL."""
        data = {"url": url}
        ...
        if chunks_per_source is not None:
            data["chunks_per_source"] = chunks_per_source
        return await self._post("crawl", data, timeout=min(timeout, 150))
    ...
    async def map(
        ...
        if include_usage is not None:
            data["include_usage"] = include_usage
        return await self._post("map", data, timeout=min(timeout, 150))
    ...
    async def research(
        ...
        """POST /research — Research task. Non-streaming only (stream=False)."""
        ...
        return await self._post("research", data, timeout=to)

    async def get_research(self, request_id: str) -> dict:
        """GET /research/{request_id} — Get research result by request_id."""
        return await self._get(f"research/{request_id}", timeout=30)
    ...
    async def get_search_context(
        ...
        """Search and return JSON string of url+content context (simplified; no token truncation)."""
    ...
    async def qna_search(
        ...
        """Search with include_answer=True; return answer string."""
        resp = await self.search(
            query,
            search_depth=search_depth,
            topic=topic,
    ...
    async def get_company_info(
        ...
        """Search across topics news, general, finance; merge and sort by score; return top max_results."""
