...
class SummarizationAIAgentProducer(agent.AbstractAIAgentChannelProducer):
    ...
    def _unwrap_agent_result(self, result: AgentResult) -> AgentResult:
        ...
        """
        result_key = agent.AbstractAgentChannel.RESULT_KEY
        agent_name_key = agent.AbstractAgentChannel.AGENT_NAME_KEY
        
        if result_key in result and isinstance(result.get(result_key), dict):
    ...
    async def execute(
        ...
        agent_results_list: AgentResultsList
        if typing.TYPE_CHECKING or hasattr(input_data, "values"):
            # Dict input from team (aggregated inputs)
